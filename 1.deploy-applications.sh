#0 Demo project
oc new-project chat --display-name="Philly Convergence Event - Chat Program" 
...
## Ingress
oc expose  svc istio-ingressgateway --name chat-client-ingress --hostname="chat-client.apps.philly.openshiftworkshop.com" -n istio-system
oc expose  svc istio-ingressgateway --name chat-server-ingress --hostname="chat-server.apps.philly.openshiftworkshop.com" -n istio-system
